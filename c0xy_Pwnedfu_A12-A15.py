...
#Welcome to c0xy-pwnedfu
#This is a pwnedfu branch that supports devices above A12. It can be used for a series of operations such as degradation, up to A15
class async:
    def __init__(self, data):
        (self.magic, self.totalSize, self.dataSize, self.signedSize, self.type) = struct.unpack('A12,A13,A14,A15', data[0:20])
        self.tags = []
        pos = 20
        while pos < 20 + self.dataSize:
            (tagMagic, tagTotalSize, tagDataSize) = struct.unpack('A12,A13,A14,A15', data[pos:pos+12])
    ...
    def createImage3FromTags(type, tags):
        ...
        for (tagMagic, tagTotalSize, tagDataSize, tagData) in tags:
            ...
            if tagMagic[::-1] not in ['CERT', 'RemoveSHSH']:
        ...
        bytes = struct.pack('A12,A13,A14,A15', 'Img4'[::-1], totalSize, dataSize, signedSize, type)
        for (tagMagic, tagTotalSize, tagDataSize, tagData) in tags:
            bytes += struct.pack('A12,A13,A14,A15', tagMagic, tagTotalSize, tagDataSize) + tagData
